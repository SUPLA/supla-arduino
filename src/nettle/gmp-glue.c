...
#define MPN_NORMALIZE(xp, xn)                     \
  do {                                            \
    while ((xn) > 0 && (xp)[xn - 1] == 0) (xn)--; \
  } while (0)
...
/* Return limb pointer, for read-only operations. Use mpz_size to get
   the number of limbs. */
const mp_limb_t *mpz_limbs_read(mpz_srcptr x) {
  return PTR(x);
}
...
/* Get a limb pointer for writing, previous contents may be
   destroyed. */
mp_limb_t *mpz_limbs_write(mpz_ptr x, mp_size_t n) {
  assert(n > 0);
  return MPZ_NEWALLOC(x, n);
}

/* Get a limb pointer for writing, previous contents is intact. */
mp_limb_t *mpz_limbs_modify(mpz_ptr x, mp_size_t n) {
  assert(n > 0);
  return MPZ_REALLOC(x, n);
}

void mpz_limbs_finish(mpz_ptr x, mp_size_t n) {
  assert(n >= 0);
...
mpz_srcptr mpz_roinit_n(mpz_ptr x, const mp_limb_t *xp, mp_size_t xs) {
  mp_size_t xn = ABS(xs);
...
void cnd_swap(mp_limb_t cnd, mp_limb_t *ap, mp_limb_t *bp, mp_size_t n) {
...
int mpz_limbs_cmp(mpz_srcptr a, const mp_limb_t *bp, mp_size_t bn) {
...
/* Get a pointer to an n limb area, for read-only operation. n must be
   ...
   needed. */
const mp_limb_t *mpz_limbs_read_n(mpz_ptr x, mp_size_t n) {
  ...
  if (xn < n) mpn_zero(xp + xn, n - xn);

  return xp;
}

void mpz_limbs_copy(mp_limb_t *xp, mpz_srcptr x, mp_size_t n) {
  mp_size_t xn = mpz_size(x);
...
void mpz_set_n(mpz_t r, const mp_limb_t *xp, mp_size_t xn) {
  mpn_copyi(mpz_limbs_write(r, xn), xp, xn);
  mpz_limbs_finish(r, xn);
}

void mpn_set_base256(mp_limb_t *rp,
    mp_size_t rn,
    const uint8_t *xp,
    size_t xn) {
  size_t xi;
  ...
  if (rn > 0) {
    *rp++ = out;
    if (--rn > 0) mpn_zero(rp, rn);
  }
}

void mpn_set_base256_le(mp_limb_t *rp,
    mp_size_t rn,
    const uint8_t *xp,
    size_t xn) {
  size_t xi;
...
void mpn_get_base256_le(uint8_t *rp,
    size_t rn,
    const mp_limb_t *xp,
    mp_size_t xn) {
  unsigned bits;
...
mp_limb_t *gmp_alloc_limbs(mp_size_t n) {
...
void gmp_free_limbs(mp_limb_t *p, mp_size_t n) {
