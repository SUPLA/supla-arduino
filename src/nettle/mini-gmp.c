...
#define gmp_clz(count, x)                                               \
  do {                                                                  \
    ...
    for (__clz_c = 0;                                                   \
        ...
        __clz_c += 8)                                                  \
    __clz_x <<= 8;                                                    \
    for (; (__clz_x & GMP_LIMB_HIGHBIT) == 0; __clz_c++) __clz_x <<= 1; \
    (count) = __clz_c;                                                  \
  } while (0)
...
static void *gmp_default_realloc(void *old, size_t old_size, size_t new_size) {
  ...
  if (!p) gmp_die("gmp_default_realoc: Virtual memory exhausted.");
...
static void *(*gmp_reallocate_func)(void *,
    size_t,
...
int mpn_cmp(mp_srcptr ap, mp_srcptr bp, mp_size_t n) {
  while (--n >= 0) {
    if (ap[n] != bp[n]) return ap[n] > bp[n] ? 1 : -1;
  }
  return 0;
}
...
mp_limb_t mpn_add(
    mp_ptr rp, mp_srcptr ap, mp_size_t an, mp_srcptr bp, mp_size_t bn) {
  mp_limb_t cy;
...
mp_limb_t mpn_mul(
    mp_ptr rp, mp_srcptr up, mp_size_t un, mp_srcptr vp, mp_size_t vn) {
  assert(un >= vn);
  assert(vn >= 1);
...
static mp_bitcnt_t mpn_common_scan(
    mp_limb_t limb, mp_size_t i, mp_srcptr up, mp_size_t un, mp_limb_t ux) {
  unsigned cnt;
...
static mp_limb_t mpn_div_qr_1_preinv(mp_ptr qp,
  ...
  if (inv->shift > 0) gmp_free(tp);

  return r >> inv->shift;
}

static mp_limb_t mpn_div_qr_1(mp_ptr qp,
    mp_srcptr np,
    mp_size_t nn,
    mp_limb_t d) {
  assert(d > 0);
...
static void mpn_div_qr_preinv(mp_ptr qp,
  ...
  if (dn == 1)
    ...
    if (shift > 0) gmp_assert_nocarry(mpn_rshift(np, np, dn, shift));
  }
}

static void mpn_div_qr(
    mp_ptr qp, mp_ptr np, mp_size_t nn, mp_srcptr dp, mp_size_t dn) {
  struct gmp_div_inverse inv;
  mp_ptr tp = NULL;
...
static size_t mpn_get_str_other(unsigned char *sp,
  ...
  if (un > 1) {
    ...
    do {
      ...
      for (sn += done; done < info->exp; done++) sp[sn++] = 0;
    } while (un > 1);
  }
  ...
  for (j = sn, rn = 0, shift = 0; j-- > 0;) {
    if (shift == 0) {
      ...
      if (shift >= GMP_LIMB_BITS) {
        shift -= GMP_LIMB_BITS;
        if (shift > 0) rp[rn++] = (mp_limb_t)sp[j] >> (bits - shift);
      }
    }
  }
...
static mp_size_t mpn_set_str_other(mp_ptr rp,
  ...
  for (rn = (w > 0); j < sn;) {
    ...
    w = sp[j++];
    for (k = 1; k < info->exp; k++) w = w * b + sp[j++];
...
static void *mpz_realloc(mpz_t r, mp_size_t size) {
  ...
  if (GMP_ABS(r->_mp_size) > size) r->_mp_size = 0;
...
int mpz_cmpabs(const mpz_t u, const mpz_t v) {
  return mpn_cmp4(
      u->_mp_d, GMP_ABS(u->_mp_size), v->_mp_d, GMP_ABS(v->_mp_size));
}
...
static int mpz_div_qr(mpz_t q,
  ...
  if (ds == 0) gmp_die("mpz_div_qr: Divide by zero.");
  ...
  if (nn < dn) {
    ...
    if (mode == GMP_DIV_FLOOR && qs < 0 && rn != 0) {
      if (q) mpz_sub_ui(tq, tq, 1);
      if (r) mpz_add(tr, tr, d);
    } else if (mode == GMP_DIV_CEIL && qs >= 0 && rn != 0) {
      if (q) mpz_add_ui(tq, tq, 1);
      if (r) mpz_sub(tr, tr, d);
    }
...
static void mpz_div_r_2exp(mpz_t r,
  ...
  if (rn > un) {
    ...
    if (mode == ((us > 0) ? GMP_DIV_CEIL : GMP_DIV_FLOOR)) /* us != 0 here. */
    {
      ...
      assert(cy == 0);
      for (; i < rn - 1; i++) rp[i] = GMP_LIMB_MAX;

      rp[rn - 1] = mask;
      us = -us;
    } else {
      /* Just copy */
      if (r != u) mpn_copyi(rp, u->_mp_d, un);

      rn = un;
    }
  } else {
    if (r != u) mpn_copyi(rp, u->_mp_d, rn - 1);
...
int mpz_congruent_p(const mpz_t a, const mpz_t b, const mpz_t m) {
  ...
  /* a == b (mod 0) iff a == b */
  if (mpz_sgn(m) == 0) return (mpz_cmp(a, b) == 0);
...
static unsigned long mpz_div_qr_ui(mpz_t q,
  ...
  if (rl > 0 && ((mode == GMP_DIV_FLOOR && ns < 0) ||
    ...
    if (q) gmp_assert_nocarry(mpn_add_1(qp, qp, qn, 1));
    rl = d - rl;
    rs = -rs;
  }
...
unsigned long mpz_gcd_ui(mpz_t g, const mpz_t u, unsigned long v) {
  ...
  if (v == 0) {
    ...
    un = GMP_ABS(u->_mp_size);
    if (un != 0) v = mpn_gcd_11(mpn_div_qr_1(NULL, u->_mp_d, un, v), v);
...
void mpz_gcd(mpz_t g, const mpz_t u, const mpz_t v) {
  ...
  if (tu->_mp_size < tv->_mp_size) mpz_swap(tu, tv);
...
void mpz_gcdext(mpz_t g, mpz_t s, mpz_t t, const mpz_t u, const mpz_t v) {
  ...
  if (u->_mp_size < 0) mpz_neg(s0, s0);
  if (v->_mp_size < 0) mpz_neg(t0, t0);
...
int mpz_invert(mpz_t r, const mpz_t u, const mpz_t m) {
  ...
  if (u->_mp_size == 0 || mpz_cmpabs_ui(m, 1) <= 0) return 0;
...
void mpz_powm(mpz_t r, const mpz_t b, const mpz_t e, const mpz_t m) {
  ...
  if (mn == 0) gmp_die("mpz_powm: Zero modulo.");
...
void mpz_rootrem(mpz_t x, mpz_t r, const mpz_t y, unsigned long z) {
  ...
  if (z == 0) gmp_die("mpz_rootrem: Zeroth root.");
...
mp_size_t mpn_sqrtrem(mp_ptr sp, mp_ptr rp, mp_srcptr p, mp_size_t n) {
  ...
  mpz_clear(s);
  res = r->_mp_size;
  if (rp) mpn_copyd(rp, r->_mp_d, res);
  mpz_clear(r);
  return res;
}
...
void mpz_bin_uiui(mpz_t r, unsigned long n, unsigned long k) {
  ...
  if (k > (n >> 1)) k = (k <= n) ? n - k : 0;

  mpz_init(t);
  mpz_fac_ui(t, k);

  for (; k > 0; k--) mpz_mul_ui(r, r, n--);
...
/* Primality testing */
static int gmp_millerrabin(
    const mpz_t n, const mpz_t nm1, mpz_t y, const mpz_t q, mp_bitcnt_t k) {
  assert(k > 0);

  /* Caller must initialize y to the base. */
  mpz_powm(y, y, q, n);

  if (mpz_cmp_ui(y, 1) == 0 || mpz_cmp(y, nm1) == 0) return 1;
...
int mpz_probab_prime_p(const mpz_t n, int reps) {
  ...
  /* Note that we use the absolute value of n only, for compatibility
     with the real GMP. */
  if (mpz_even_p(n)) return (mpz_cmpabs_ui(n, 2) == 0) ? 2 : 0;
  ...
  if (mpz_gcd_ui(NULL, n, GMP_PRIME_PRODUCT) != 1) return 0;

  /* All prime factors are >= 31. */
  if (mpz_cmpabs_ui(n, 31 * 31) < 0) return 2;
...
int mpz_tstbit(const mpz_t d, mp_bitcnt_t bit_index) {
  ...
  if (ds < 0) {
    ...
    if (shift > 0 && (w << (GMP_LIMB_BITS - shift)) > 0) return bit ^ 1;
    while (limb_index-- > 0)
      if (d->_mp_d[limb_index] > 0) return bit ^ 1;
  }
  return bit;
}
...
static void mpz_abs_add_bit(mpz_t d, mp_bitcnt_t bit_index) {
  ...
  if (limb_index >= dn) {
    ...
    dp[limb_index] = bit;
    for (i = dn; i < limb_index; i++) dp[i] = 0;
    dn = limb_index + 1;
  } else {
    mp_limb_t cy;
...
mp_bitcnt_t mpn_popcount(mp_srcptr p, mp_size_t n) {
  ...
  for (c = 0, i = 0; i < n; i++) c += gmp_popcount_limb(p[i]);
...
mp_bitcnt_t mpz_hamdist(const mpz_t u, const mpz_t v) {
  ...
  if ((un ^ vn) < 0) return ~(mp_bitcnt_t)0;
  ...
  if (un < vn) MPN_SRCPTR_SWAP(up, un, vp, vn);
...
mp_bitcnt_t mpz_scan1(const mpz_t u, mp_bitcnt_t starting_bit) {
  ...
  if (i >= un) return (us >= 0 ? ~(mp_bitcnt_t)0 : starting_bit);
...
mp_bitcnt_t mpz_scan0(const mpz_t u, mp_bitcnt_t starting_bit) {
  ...
  if (i >= un) return (ux ? starting_bit : ~(mp_bitcnt_t)0);

  up = u->_mp_d;
  limb = up[i] ^ ux;

  if (ux == 0) limb -= mpn_zero_p(up, i); /* limb = ~(~limb + zero_p) */
...
char *mpz_get_str(char *sp, int base, const mpz_t u) {
  ...
  for (; i < sn; i++) sp[i] = digits[(unsigned char)sp[i]];
...
void mpz_import(mpz_t r,
  ...
  if (nails != 0) gmp_die("mpz_import: Nails not supported.");
