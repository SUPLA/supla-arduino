...
//                crc = _crc16_update(crc, vptr[i]);
        ...
        if (crc != eepromcrc) { // implement check if CRC is OK
            supla_log(LOG_DEBUG, "Incorrect CRC: calaculated %d; read from EEPROM %d", crc, eepromcrc);
            ptr = firstCounter;
            while (ptr != NULL) {
                ptr->setCounter(0);
                ptr = ptr->nextCounter;
...
SuplaImpulseCounter::SuplaImpulseCounter(int _channelNumber, int _impulsePin, int _statusLedPin, bool _detectLowToHigh, bool _inputPullup, unsigned long _debounceDelay) {
    ...
    supla_log(LOG_DEBUG, "Creating Impulse Counter: impulsePin(%d), statusLedPin(%d), delay(%d ms)", impulsePin, statusLedPin, debounceDelay);
    if (impulsePin <= 0) {
        supla_log(LOG_DEBUG, "SuplaImpulseCounter ERROR - incorrect impulse pin number");
        return;
    } 
...
void SuplaImpulseCounter::debug() {
    supla_log(LOG_DEBUG, "SuplaImpulseCounter - channel(%d), impulsePin(%d), counter(%d)", channelNumber, impulsePin, static_cast<int>(counter));
}

void SuplaImpulseCounter::setCounter(_supla_int64_t value) {
    counter = value;
    isValueChanged = true;
    supla_log(LOG_DEBUG, "SuplaImpulseCounter - set counter to %d", static_cast<int>(counter));
}
