...
typedef union 
{
	struct {uint8_t byte1; uint8_t byte2;} svalue;
	uint8_t tvalue[2];
	uint16_t ivalue;
}somfy_rollingcode_t;
typedef union 
{
	struct {uint8_t byte1; uint8_t byte2; uint8_t byte3;} svalue;
	uint8_t tvalue[3];
	uint32_t ivalue : 24;
}somfy_remotesn_t;
...
enum /*class*/ ControlButtons
{
	...
	MYDOWN = 0x5,    //[My + Down]   Set lower motor limit in initial programming mode
	UPDOWN = 0x6,    //[Up + Down]   Change motor limit and initial programming mode
	PROG   = 0x8,    //[Prog]        Registering / Deregistering remotes
	SUN    = 0x9,    //[Sun + Flag]  Enable sun and wind detector (SUN and FLAG TIME on the Telis Soliris RC)
	FLAG   = 0xA     //[Flag]        Disable sun detector (FLAG TIME on the Telis Soliris RC)  
};
